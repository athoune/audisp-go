...
func messagesFactory() *message.MessagesMockup {
	return message.NewMessagesMockup(
...
func TestSyscall(t *testing.T) {
	messages := messagesFactory()
	reader, err := New(`line.syscall == syscall("execve")`, true, message.New(messages))
	assert.NoError(t, err)
	cpt := 0
	for reader.Next() {
		msg := reader.Message()
...
func TestUnknownSyscall(t *testing.T) {
	messages := messagesFactory()
	reader, err := New(`line.syscall == syscall("beuha")`, true, message.New(messages))
	assert.NoError(t, err)
	cpt := 0
	for reader.Next() {
		msg := reader.Message()
